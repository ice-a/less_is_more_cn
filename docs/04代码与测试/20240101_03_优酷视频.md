...
class YoukuVideoDownloader:
    ...
    def download_and_merge(self, url, title):
        # 获取视频 ID
        video_id = self.extract_id_from_url(url)
        if not video_id:
            print("无法获取视频ID，请检查URL是否正确")
                        ...
                        with open(filename, 'wb') as f:
                            ...
                            for chunk in response.iter_content(chunk_size=4096):  # 限制块的大小
                                if chunk:
                                    ...
                                    print("Download", downloaded, total_length)

                        video_files.append(filename)

                if video_files:
                    # 合并视频
                    output_filename = os.path.join(download_dir, f"video_{title}_merged.mp4")
                    self.merge_videos(video_files, output_filename)
                    # progress_callback(f"合并后的视频已保存为 {output_filename}", "success")
                    print(f"合并后的视频已保存为 {output_filename}", "success")
                else:
                    # progress_callback("没有可用的视频链接，无法合并视频", "error")
                    print("没有可用的视频链接，无法合并视频", "error")
            else:
                # progress_callback("未找到可用的视频标签或此视频需要VIP", "error")
                print("未找到可用的视频标签或此视频需要VIP", "error")

            # 关闭浏览器
            browser.close()


youku = YoukuVideoDownloader()
info = "甜心格格"
with open("1.txt", "r", encoding="utf-8") as f:
    for index,line in enumerate(f.readlines()):
        line = line.strip()
        if line:
            youku.download_and_merge(line, info+"_"+str(index))
